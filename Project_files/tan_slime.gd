...
signal send_attack(pos, overlap_pos)
...
func _on_player_did_a_move(_position):
	...
	if movecount % 2 == 0:
		...
		emit_signal("send_attack", position, position+position)
	
func choose_a_valid_move(_position):
	if tween.is_active():
		return
	var possible_directions = []
	var potential_directions = [Vector2.RIGHT,Vector2.LEFT,Vector2.UP,Vector2.DOWN]
...
func _on_schwoomptimer_timeout():
